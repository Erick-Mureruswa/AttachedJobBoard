# 2c9fab3c-8422-47f3-bf3c-e0d5a228d590 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `axiom-landing.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `axiom-landing.html`
...
- A single self-contained `axiom-landing.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `axiom-landing.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `axiom-landing.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
